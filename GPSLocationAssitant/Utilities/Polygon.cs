...
namespace LocationTriggering.Utilities
{
    ...
    public class Polygon
    {
        ...
        public Polygon(PointD[] points)
        {
            ...
            {
                //Iterate through the points and obtain the extremnes of the polygon
                if (P.X < MinLon)
                {
                    MinLon = P.X;
                    LeftPoint = P;
            ...
            foreach (PointD P in Points)
            {
                //Iterate through the points and get the total Latitude and Longitude to calculate centre point
                LonTotal += P.X;
                LatTotal += P.Y;
            }
            if (_crossesInternationalDateLine&&LonTotal / points.Length >180)
            {
                _centre = new PointD(-360+LonTotal / points.Length, LatTotal / points.Length);//find the average of the latitudes and longitudes to determine the centre
            }
            else
            {
                _centre = new PointD(LonTotal / points.Length, LatTotal / points.Length);//find the average of the latitudes and longitudes to determine the centre
            }
        }
